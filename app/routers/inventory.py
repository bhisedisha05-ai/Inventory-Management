...
# ✅ Just add decorator (router already exists)
@router.put("/shift-timestamps")
async def shift_timestamps(db: AsyncSession = Depends(database.get_db)):

    # 🔥 Dynamic smart range (past + current + near future)
    start_date = datetime.now() - timedelta(days=30)
    end_date = datetime.now() + timedelta(days=5)

    # 🔹 Step 1: Fetch records in range
    result = await db.execute(
        select(models.StockTransaction)
        .where(models.StockTransaction.timestamp.between(start_date, end_date))
    )
    transactions = result.scalars().all()

    # 🔹 Step 2: Update timestamps
    for txn in transactions:
        txn.timestamp = txn.timestamp + timedelta(days=2)

    await db.commit()

    # 🔹 Step 3: Fetch UPDATED data with item details
    updated_result = await db.execute(
        ...
        .join(models.Item, models.Item.id == models.StockTransaction.item_id)
        .where(
            models.StockTransaction.timestamp.between(
                start_date,
    ...
    updated_rows = updated_result.all()
    ...
    return {
        ...
        "updated_stock_movements": updated_data
    }